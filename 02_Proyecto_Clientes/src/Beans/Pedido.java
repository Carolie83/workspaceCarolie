...
public class Pedido {
	...
	public Pedido damePedido() {
		...
		Pedido [] pedidos = { new Pedido(idPedido, descripcion, importe, conIva, cliente),
		...
		public double totalImporte(int iva) {
			if (!conIva)  //significa si no con iva (!con iva) ...ej: System.out.printLn (5>4) devuelve true
				return importe;
				//otra forma : if (conIva != true) o if (conIva==false)
			else
